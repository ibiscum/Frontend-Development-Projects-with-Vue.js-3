describe("Email input", () => {
  ...
  it("add comment button should be disabled when no email", () => {
    cy.visit("/");
    cy.get('[data-test-id="email-input"]').should("have.value", "");

    cy.get('[data-test-id="new-comment-button"]').should("be.disabled");

    cy.get('[data-test-id="email-input"]').type("hugo@example.tld");
  ...
  it("when adding comment, it should be created with the input email", () => {
    ...
    cy.wait("@newComment").its("request.body").should("deep.equal", {
      body: "My new comment",
      email: "hugo@example.tld",
    });
  });
