  ...
  <div id="app" class="p-10">
    <div class="flex flex-col">
      ...
      <div class="flex flex-col mx-auto mb-4">
        ...
        <input id="email" v-model="email" type="email" data-test-id="email-input"
      ...
      <button class="flex mx-auto bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
        data-test-id="new-comment-button" :disabled="!email" :class="{ 'opacity-50 cursor-not-allowed': !email }"
        @click="showEditor = !showEditor">
        Add a New Comment
      </button>
      <div v-if="showEditor">
        <textarea v-model="newComment" data-test-id="new-comment-editor"
        ...
        <button data-test-id="new-comment-submit"
          ...
          :disabled="!newComment || isSubmitting" :class="{
      ...
      <ul v-if="comments.length > 0" class="flex flex-col items-center my-4 mx-auto md:w-2/3 lg:w-1/2"
        data-test-id="comments-list">
        <li v-for="(comment, index) in comments" :key="comment.id + index"
          class="flex flex-col w-full px-6 py-4 rounded overflow-hidden shadow-lg mb-6" data-test-id="comment-card">
          <p class="flex text-gray-700 text-lg mb-4">
            {{ comment.body }}
          </p>
